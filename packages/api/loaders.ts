...
const topTrackLoader: DataLoader<string, TopTrack[]> = new DataLoader(
  (usernames: readonly string[]): Promise<TopTrack[][]> =>
    Promise.all(
      usernames.map(
...
const topArtistsLoader: DataLoader<string, TopArtist[]> = new DataLoader(
  (usernames: readonly string[]): Promise<TopArtist[][]> =>
    Promise.all(
      usernames.map(
...
const artistInfoLoader: DataLoader<string, ArtistInfo> = new DataLoader(
  (artists: readonly string[]): Promise<ArtistInfo[]> =>
    Promise.all(
      artists.map(
...
const artistLoader: DataLoader<string, ArtistBase> = new DataLoader(
  (artists: readonly string[]): Promise<ArtistBase[]> =>
...
const topAlbumsLoader: DataLoader<string, TopAlbum[]> = new DataLoader(
  (usernames: readonly string[]): Promise<TopAlbum[][]> =>
    Promise.all(
      usernames.map(
        (username: string): Promise<TopAlbum[]> =>
          lastFm.getTopAlbums(username).then(({ topalbums }): TopAlbum[] => {
            topalbums.album.forEach((album): void => {
...
const trackLoader: DataLoader<{ trackName: string; artistName: string }, TrackInfo> = new DataLoader(
  (tracks: readonly { trackName: string; artistName: string }[]): Promise<TrackInfo[]> =>
    Promise.all(
